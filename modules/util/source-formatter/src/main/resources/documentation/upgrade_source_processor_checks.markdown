...
UpgradeJavaCPInstanceServicesCheck | [Upgrade](upgrade_checks.markdown#upgrade-checks) | Fill the new parameters of the method `fetchByExternalReferenceCode` and `fetchCPInstanceByExternalReferenceCode` of `CPInstanceLocalService`, `CPInstanceLocalServiceUtil`, `CPInstanceService` and `CPInstanceServiceUtil` classes |

...
UpgradeJavaFetchAssetVocabularyByExternalReferenceCodeCheck | [Upgrade](upgrade_checks.markdown#upgrade-checks) | Adds a message about the breaking change made on method `fetchAssetVocabularyByExternalReferenceCode` of `AssetVocabularyLocalService` and `AssetVocabularyLocalServiceUtil` classes. |

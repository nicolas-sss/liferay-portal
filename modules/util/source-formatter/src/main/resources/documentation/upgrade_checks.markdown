...
UpgradeJavaCPInstanceServicesCheck | .bnd, .ftl, .gradle, .java, .json, .jsp, .jspf, .scss or .vm | Fill the new parameters of the method `fetchByExternalReferenceCode` and `fetchCPInstanceByExternalReferenceCode` of `CPInstanceLocalService`, `CPInstanceLocalServiceUtil`, `CPInstanceService` and `CPInstanceServiceUtil` classes |

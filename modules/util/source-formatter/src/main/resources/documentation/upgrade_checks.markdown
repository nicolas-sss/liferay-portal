...
UpgradeJavaFetchAssetVocabularyByExternalReferenceCodeCheck | .bnd, .ftl, .gradle, .java, .json, .jsp, .jspf, .scss or .vm | Adds a message about the breaking change made on method `fetchAssetVocabularyByExternalReferenceCode` of `AssetVocabularyLocalService` and `AssetVocabularyLocalServiceUtil` classes. |

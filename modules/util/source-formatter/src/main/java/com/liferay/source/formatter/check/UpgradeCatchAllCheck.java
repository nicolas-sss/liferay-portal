...
public class UpgradeCatchAllCheck extends BaseFileCheck {
	...
	private static Pattern _getMethodSignaturePattern(JSONObject jsonObject) {
		...
		String regex = StringUtil.replace(from, unescaped, escaped);

		regex = regex + ")(?:\\s+throws\\s+[A-Za-z0-9._<>\\s,]+)?";
	...
	private String _formatJava(
		...
		JSONArray methodsToFormatJSONArray = jsonObject.getJSONArray(
			"methodsToFormat");

		if (JSONUtil.isEmpty(methodsToFormatJSONArray)) {
			return newContent;
		}

		for (Object method : methodsToFormatJSONArray) {
	...
	private String _insertMethodAlphabetically(
		...
		if (classDeclarationMatcher.find()) {
			startIndex = classDeclarationMatcher.end() + 1;
		...
		while (bodyMatcher.find()) {
			...
			int methodEndIndexInBody = _findMatchingClosingBrace(
				classBody, bodyMatcher.end() - 1);

			if (methodEndIndexInBody != -1) {
				existingMethods.add(
					classBody.substring(methodIndex, methodEndIndexInBody + 1));
		...
		Matcher newMethodMatcher = methodPattern.matcher(newMethod);
